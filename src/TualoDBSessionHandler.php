...
class TualoDBSessionHandler implements \SessionHandlerInterface, \SessionIdInterface
{
  ...
  private function newDBByRow($row)
  {
    if (defined('__DB_SSL_KEY__') && defined('__DB_SSL_CERT__') && defined('__DB_SSL_CA__')) {
      return new Database($row['db_user'], $row['db_pass'], $row['db_name'], $row['db_host'], $row['db_port'], __DB_SSL_KEY__, __DB_SSL_CERT__, __DB_SSL_CA__);
    }
    return new Database($row['db_user'], $row['db_pass'], $row['db_name'], $row['db_host'], $row['db_port']);
  }

  function __construct()
  {
    if (!defined('__USE_DB_SESSION_HANLDER_ENCRYPTION__')) {
      define('__USE_DB_SESSION_HANLDER_ENCRYPTION__', 'shouldbesecret');
    }
    $this->db = $this->newDBByRow([
      'db_host' => __SESSION_HOST__,
      'db_pass' => __SESSION_PASSWORD__,
      'db_user' => __SESSION_USER__,
      'db_name' => __SESSION_DSN__,
      'db_port' => __SESSION_PORT__
    ]);
  }
